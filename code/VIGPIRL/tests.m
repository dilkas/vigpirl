...
function test_remaining_vigpirlkernel(testCase)
  ...
  mdp_model = 'linearmdp';
  counts = [1; 0; 0];
  estimated_grad = [-6; 15; 0; -6; 12];

  answer = estimate_derivative(u, KruKuu, Kuu, Kuuinv, r_covariance_matrix,...
    Kuu_grad, mdp, Sigma, Sigma_inv, mu, mdp_model, demonstrations, counts);
  ...
  grad = full_gradient(Sigma, Sigma_inv, mdp_model, mdp, demonstrations, counts, mu,...
