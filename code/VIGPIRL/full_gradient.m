function changes = full_gradient(Sigma, Sigma_inv, mdp_model,...
  ...
  % TEMP FOR TESTING
  %not_estimated_lambda1 = arrayfun(@(i) counts' * (Kru_grad(:, :, i)' -...
  %  Kru' * Kuu_inv * Kuu_grad(:, :, i)) * Kuu_inv * mu, 1:size(Kuu_grad, 3));
  %not_estimated_lambda2 = arrayfun(@(i) 0.5 * (trace(Kuu_inv * Kuu_grad(:, :, i) * Kuu_inv * Sigma) +...
  %  mu' * Kuu_inv * Kuu_grad(:, :, i) * Kuu_inv * mu -...
  %  trace(Kuu_inv * Kuu_grad(:, :, i))), 1:size(Kuu_grad, 3));
  %disp(not_estimated_lambda1);
  %disp(not_estimated_lambda2);
  ...
  not_estimated_elbo = counts' * Kru' * Kuu_inv * mu -...
    0.5 * (trace(Kuu_inv * Sigma) + mu' * Kuu_inv * mu + log(det(Kuu)) - log(det(Sigma)));
