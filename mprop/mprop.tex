...
%Imagine a scenario where the agent is driving on a three-lane highway. The
%features of the environment include the agent's speed, lane number, and
%various distances to different types of vehicles in front of and behind the
%agent. The agent can regulate its speed and position. The main objective is, of
%course, to not crash into other vehicles, but one could also add objectives such
%as staying within the speed limit when close to a police vehicle.
...
For many of the variables in our model (including the inducing points
located somewhere in $\mathbb{R}^d$ and the hyperparameters of the covariance
function), we have three possible ways of handling them:
\begin{itemize}
...
% TODO: the real reason is that we want to use the prior
...
% what if the parameters of the covariance function are variational?

%mTKuum, log|Kuu|, Tr[KuuS] => p(u | lambda, X_u)
%tTKruKuum, V_r(s), T(s, a, s')V_r(s) => p(D | r) (DTC?)
