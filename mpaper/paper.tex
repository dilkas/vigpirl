...
a sketch proof of the measurability of MDP value functions, which is non-obvious
due to their non-trivial definition. Afterwards, we establish bounds for the
value functions, and, after another quick lemma, tackle the main proof of this
paper. See Figure \ref{fig:theorems} for an overview of how these results fit
together.
...
% TODO: too long, lemma could be replaced by the continuity argument (and arrow
% removed from the figure), needs more explanations. Otherwise quite tolerable.
...
\begin{proof}
  ...
  First, we can easily deduce that
  ...
  \[
    \left. \frac{\partial \mathbf{K}}{\partial \lambda_0} \right|_{\lambda_0 =
      c(\mathbf{r}, \mathbf{u})}
  \]
  as an expression has no $\lambda_0$ in it, so
  \[
    \left. \frac{\partial \mathbf{K}}{\partial \lambda_0} \right|_{\lambda_0 =
  ...
  \]
  so the same reasoning as for \eqref{eq:convergence1} applies.
  ...
  \begin{align*}
    ...
    \end{multlined}
  \end{align*}
  Thus, we can express $\mathbf{K}|_{\lambda_i = c(\mathbf{r}, \mathbf{u})}$ as
  $\mathbf{K}|_{\lambda_i = c(\mathbf{r}, \mathbf{u})} = \mathbf{K} +
  \mathbf{E}$, where $\mathbf{E}$ is defined by
  \[
  ...
  \]
  Then, since $\mathbf{K}$ is invertible,
  \[
    \mathbf{K}^{-1}_{\lambda_i = c(\mathbf{r}, \mathbf{u})} \to \mathbf{K}^{-1}
  \]
  by continuity of $\mathbf{A} \mapsto \mathbf{A}^{-1}$.
