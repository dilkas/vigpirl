...
\lambda_d)^\intercal$. % TODO: write about lambda having to be variational
...
\[ \nabla_{\bm\nu} \mathcal{L} = \mathbb{E}[\nabla_{\bm\nu} \log \approximation
  (\log \pfull - \log \approximation)]. \]
The gradient of the ELBO then has a unbiased estimate
\[
  \nabla_{\bm\nu} \mathcal{L} \approx \frac{1}{S} \sum_{s=1}^S \nabla_{\bm\nu} \log \approximationS (\log \pfullS - \log \approximationS),
\]
computed by drawing $S$ Monte Carlo samples $(\mathbf{u}_s, \mathbf{r}_s) \sim
\approximation$.
