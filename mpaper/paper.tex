...
In order to fully understand the model's behaviour, we start with a three-state
...
\ref{fig:convergence1}\footnote{In all experiments, we discard runs that
...
\begin{figure}
  ...
  \caption{Box plots of the difference between (absolute values of) reward
    covariances in random and semi-structured scenarios from Figure
    \ref{fig:cliques}, averaged out over 100 runs and grouped into four key
    categories. `First state' refers to the variance of $r(s_0)$, the reward of
    the state targeted by all demonstrations in the semi-structured case. `Other
    states' are variances of other states of the MDP, i.e., diagonal entries of
    the covariance matrix. `Incident edges' refers to covariances between
    $r(s_0)$ and all other states, i.e., the first row or column of the
