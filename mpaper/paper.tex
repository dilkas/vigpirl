...
\[
  \begin{split}
    ...
    \mathbf{u}) + \log p(\mathcal{D} \mid \mathbf{r})] \\
...
\[
  \begin{split}
    ...
    &= -\frac{1}{2} (\tr (\Kuu^{-1}\bm\Sigma) +
      \bm\mu^\intercal\Kuu^{-1}\bm\mu - m \\
...
\[
  \begin{split}
    \mathcal{L} &= \mathbb{E}\left[ \sum_{i=1}^N \sum_{t=1}^T
      Q_{\mathbf{r}}(s_{i,t}, a_{i,t}) - \V(s_{i,t}) \right] \\
...
\[
  \begin{split}
  \mathcal{L} &= \mathbf{t}^\intercal\Kru^\intercal\Kuu^{-1}\bm\mu -
  \mathbb{E}[v] \\
  &- \frac{1}{2} \left(\tr \left( \Kuu^{-1}\bm\Sigma \right) +
    \bm\mu^\intercal\Kuu^{-1}\bm\mu + \log |\Kuu| - \log |\bm\Sigma| \right),
  \end{split}
...
\begin{proof}
  ...
  \begin{alignat*}{3}
    k_{\bm\lambda}(\mathbf{x}_j, \mathbf{x}_k)|_{\lambda_i = c(\mathbf{r},
