...
state-action pairs.
% TODO (must): along with inverse optimal control, mention imitation learning
...
the first paragraph. Quite often, the models assume that rewards have no
variance \cite{DBLP:conf/nips/LevinePK11,DBLP:conf/uai/JinDAS17}.
...
prove useful in four major ways:
\begin{enumerate}
...
Our main contribution is a proof in Section~\ref{sec:proof} that shows
...
\begin{definition}[IRL]
  ...
  of features), and a set of demonstrations $\mathcal{D} = \{\zeta_i
...
Our goal is then to use VI in order to approximate
$p(\mathbf{u},~\mathbf{r}~\mid~\mathcal{D})$. Let $\approximation$ denote our
approximation. VI aims to optimise this approximation by minimising the
...
complicates the situation. The goal of this section is to show how
Theorem~\ref{thm:lebesgue} can be applied to our model in order to derive the
gradient anyway\footnote{This technique is inspired by black box VI
