...
% 2. Avoid saying that epsilon is arbitrary and using phrases like 'sufficiently small'.
...
data in $\mathcal{D}$ to guide its value, so the maximum causal entropy
framework would put the probability at around $0.5$. The first thing to note is
that while $\mathcal{L}$ converges in just a few iterations, policies continue
to improve for much longer. In addition, all policies behave as expected:
$\pitwo$ stays around $0.5$, while both $\pione$ and $\pithree$ keep increasing.
Although they stay at around $0.75$ (and not 1), this may be optimal for the
maximum causal entropy model (and this question will be addressed in more detail
with a different experiment).

We also plot how the parameters of the model converge in Figure
\ref{fig:convergence2}. While some parameters could benefit from a higher number of
iterations\footnote{The limit of 300 iterations was chosen so that the initial
  increase in $\mathcal{L}$ would not be overshadowed by a long straight line
  afterwards.}, none look alarmingly wrong. Note that $\mu_2$, a variable
...
\begin{figure}[!ht]
  ...
  \begin{tikzpicture}[
    ...
    \foreach \x in {1, 2, 3} {
      \node (x\x) at (0.5+2*\x, 1.3) {$\x$};
    }
    \foreach \y in {1, 2, 3} {
      \node (y\y) at (1.3, 0.5+2*\y) {$\y$};
    }
    \begin{axis}[
      colormap/violet,
      scale only axis,
  ...
  \caption{Changes in reward covariances after adding more demonstrations. The
    ...
    colours represent median covariances across 10 runs, normalised to the
...
\end{align*}
by the identity for $\mathbb{E}[\mathbf{xx}^\intercal]$ from the book by
Petersen and Pedersen \cite{petersen2008matrix}. Now we can find the covariance
matrix as follows:
\begin{align*}
...
Unfortunately, the resulting plot in Figure \ref{fig:covariance} shows no clear
pattern. The only worthwhile observation is that there are only two instances of
negative covariance, and both of them are between states $s_2$ and $s_3$.
Perhaps there is a good (currently unknown) reason for why adding more data does
not reduce variance, or perhaps this plot could benefit from a higher number of
iterations and more repeated experiments to ensure the accuracy of the results.
% or a different MDP, like a star
...
\end{figure}
% TODO: write down all the relevant numbers
% TODO: these things are true most of the time, but not always, sometimes the
% exact opposite is true
% not showing outliers for clarity
% smaller sample of demonstrations (7 instead of 100)
% uniformly random

% EXTRA INFO:
% Discarding iterations where either $\bm\Sigma$ or $\bm\Gamma$ becomes near-singular.
%Diagonal and non-diagonal entries of the covariance matrix are normalised to
%$[0, 1]$ separately, so as to emphasize the variation in each. There were no
%negative entries of the covariance matrix after convergence.
%More data creates bigger gradients---need smaller step size. Dynamic step sizes
%don't work very well because they become zero after a big spike (and spikes do
%happen).
...
\item Reasonable results with policy convergence.
...
\item Determining how reward covariance depends on data would be the next step.
\end{itemize}

\subsection{Further Work}
%TODO: how optimal policies depend on the structure of the mdp and available
%data (i.e., why don't our policies converge to 1)
