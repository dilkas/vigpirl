...
%\begin{figure} % this is more confusing than helpful
%  \centering
%  \begin{tikzpicture}
%    \node [ellipse,draw] (s1) at (-1, -1) {$s_1$};
%    \node [ellipse,draw] (s2) at (0, 0) {$s_2$};
%    \node [ellipse,draw] (s3) at (1, -1) {$s_3$};
%    \draw [->,-to,bend left] (s1) edge (s2);
%    \draw [->,-latex,dashed] (s1) edge (s3);
%    \draw [->,-to] (s2) edge (s1);
%    \draw [->,-latex,bend left,dashed] (s2) edge (s3);
%    \draw [->,-to,bend left,dashed] (s3) edge (s1);
%    \draw [->,-latex] (s3) edge (s2);
%  \end{tikzpicture}
%\end{figure}
...
%Our work also poses a new challenge to the usual methods of determining an
%optimal strategy in a given MDP:
%\begin{problem}
%  Let $\mathcal{M} = \{ \mathcal{S}, \mathcal{A}, \mathcal{T}, \gamma,
%  r \}$ be an MDP where the reward $r : \mathcal{S} \to
%  \textsf{Prob}(\mathbb{R})$ is expressed as a function from the set of states
%  to the set of probability distributions with support over $\mathbb{R}$. Find
%  the optimal policy $\pi^*$ in $\mathcal{M}$, i.e., % TODO: argmax expression
%\end{problem}
%Of course, in many situations,... % TOOD: gaussians, pdf's, something simpler
