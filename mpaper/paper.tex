...
Most approaches in the literature make a convenient yet unjustified assumption
that the reward function can be expressed as a linear combination of features.
One proven way to abandon this assumption is by representing the reward function
...
TODO: describe the problem with B.

\begin{figure*}
  \centering
  \begin{minipage}{.5\textwidth}
    \centering
    \includegraphics[width=\columnwidth]{convergence1}
  \end{minipage}%
  \begin{minipage}{.5\textwidth}
    \centering
    \includegraphics[width=\columnwidth]{convergence2}
  \end{minipage}
...
Because of this issue, we look at the convergence of our model using a constant
step size in two different scenarios:
\begin{itemize}
...
\item In \textbf{Scenario 2}, we set $\mathbf{B} = \mathbf{I}_m$, and do not
  optimise it at all.
\end{itemize}
% TODO: use \ell when introducing the norms
...
% TODO: comprehensive literature survey
