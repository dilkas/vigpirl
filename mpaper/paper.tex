...
% TODO: in this section, 'constant' means 'constant w.r.t. u and r'
...
\begin{proof}
  ...
  First, we will show that $\mathbf{K}^{-1}|_{\lambda_i = c(\mathbf{r},
    \mathbf{u})}$ has constant bounds. If $i = 0$, then $\mathbf{K}|_{\lambda_i
    = c(\mathbf{r}, \mathbf{u})} = c(\mathbf{r}, \mathbf{u})\mathbf{A}$, where
  $\mathbf{A}$ is a constant matrix defined by
  \[
  ...
  \]
  Since $\lambda_0$ cannot be zero, we can make $\epsilon$ small enough so that
  either $\lambda_0 - \epsilon > 0$ or $\lambda_0 + \epsilon < 0$, i.e., the
  values of $c(\mathbf{r}, \mathbf{u})$ are either all positive or all negative.
  Then, by continuity of $x \mapsto \frac{1}{x}$, $\mathbf{K}^{-1}|_{\lambda_i =
    c(\mathbf{r}, \mathbf{u})}$ has constant bounds.
  ...
  \mathbf{u}) = \lambda_i + \delta$. If $i > 0$, then
  \begin{align*}
  ...
  \[
    ...
    \right).
  \]
  Note that $E_{j,k} \to 0$ as $\delta \to 0$ and $\delta$ can be made
  arbitrarily small via $\epsilon$. Then, since $\mathbf{K}$ is invertible, we
  can make $\delta$ small enough so that $\lVert \mathbf{K}^{-1} \rVert_\infty
  \lVert \mathbf{E} \rVert_\infty < 1$, and Lemma \ref{lemma:perturbation} shows
  ...
  % TODO: actually demonstrate this for the inverse
  Recall that
  \[
  ...
  = \mathbf{K}$. We can use a general fact that if $\lim_{x \to 0} f(x) = f$,
...
% Mention all the constants I've used.
...
highest.
 ...
 % TODO: need to redo this to get better labels, more precision, wider range of values
\begin{figure*}
  \centering
...
% How does policy depend on rewards? Choose state and action, generate an array
% of contour plots showing the probability of taking that action in an optimal policy
...
% How well does the variance prediction work?
