...
\begin{algorithm}
  ...
  \Function{\estimatedPart{\parameters, $\mathbf{u}$}}{
    ...
    \Return{the estimated part of $\nabla\mathcal{L}$ according to $\mathbf{r}$,
      $\V$, and \parameters\;}
  }
  \caption{The optimisation procedure. All parameters are kept in a single variable
    \textsf{parameters}. Converting it to/from vector form and using $\log$,
    $\exp$, and Equation~\eqref{eq:transformation} to handle the optimisation of
    parameters restricted to positive values is handled by functions
    \texttt{pack}, \texttt{unpack}, and \texttt{transformGradient}. Each
    derivative from Appendix~\ref{appendix:derivatives} is split into parts
    outside and inside $\mathbb{E}[\cdot]$ which are handled differently by
    functions \texttt{exactPart} and \texttt{estimatedPart}. The function
...
% TODO: must shorten something by ~1 line
\begin{figure}[!t]
  \centering
  \begin{tikzpicture}
    \foreach \i in {0,...,9}
...
\end{figure}
% TODO: probably need to conclude the experiments
