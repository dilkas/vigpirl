...
% TODO: T is not necessary because D is symmetric
  ...
  \item For $i = 0, \dots, d$,
    \[
      ...
      \frac{1}{2}q(\mathbf{r} \mid \mathbf{u}) \left( |\bm\Gamma|^{-1}
        ...
        - \mathbf{Su}) \right),
    \]
    where % TODO: move this before the lemma
    \begin{align*}
...
\[
  \begin{split}
    ...
    \right),
  \end{split}
\] % TODO: cite the wikipedia page on KL divergence
which gives
\[
...
\[
  ...
  \frac{1}{2} \left( \dS\log|\bm\Sigma| - \dS\tr \left( \Kuu^{-1} \bm\Sigma
    \right) \right)
  - \dS\mathbb{E}[v].
\]
...
\begin{gather*}
  \dB\log|\bm\Sigma| = 2\bm\Sigma^{-1}\mathbf{B}, \\
  %\dD\log|\bm\Sigma| = \mathbf{T}\mathbf{D}^\intercal +
  %\mathbf{D}^\intercal\mathbf{T}, \\
  \dB \tr \left( \Kuu^{-1} \bm\Sigma \right) = 2\Kuu^{-1}\mathbf{B},
\end{gather*}
...
\begin{gather*}
  ...
  %|\bm\Sigma|^{-1}(\adj(\mathbf{T})\mathbf{D}^\intercal + \mathbf{D}^\intercal
  %\adj(\mathbf{T})) \}v].
\end{gather*} % TODO: update the D derivative
