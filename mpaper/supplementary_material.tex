...
% TODO: move the reasoning about epsilon somewhere around here
% TODO: move (Kuu u)^T to u^T Kuu^T

% need to establish upper and lower bounds on the derivative
% can use triangle inequality: |AB| <= A'B', where A'_i = |A_i|
...
\begin{proof}
  ...
  The $i$th element of the vector
    ...
    \]
    \begin{enumerate} % TODO: update this to use the real lemma
    ...
    \item Because of the result in \ref{part_2a}, we only need to prove the
      existence of % TODO: true because each element has an absolute value upper
                   % bound, so we can replace C(u)^-T with a sufficiently big
                   % constant
      \[
